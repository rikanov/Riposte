...
class GameViewModel {
    ...
    var tournamentOpponentNameRes: StringResource? by mutableStateOf(null)
    ...
    fun requestHint() {
        ...
        viewModelScope.launch {
            ...
            val move = withContext(Dispatchers.Default) {
                NativeEngine.getBestStep(hash.p1, hash.p2, currentPlayerId, 7, settings.riposteAllowed, separationStepsLeft, 10, 10)
            }
    ...
    private fun getRandomOpeningMove(currentP1: Long, currentP2: Long, offW: Int, defW: Int): MoveData {
        ...
        return if (validMoves.isNotEmpty()) {
            ...
            NativeEngine.getBestStep(currentP1, currentP2, 1, settings.difficulty, settings.riposteAllowed, separationStepsLeft, offW, defW)
        }
    }
        ...
        viewModelScope.launch {
            ...
            val move = withContext(Dispatchers.Default) {
                ...
                if(isFirstMove) {
                    ...
                    NativeEngine.getBestStepTT(hash.p1, hash.p2, 1, settings.difficulty, settings.riposteAllowed, separationStepsLeft, currentOffW, currentDefW)
                } else {
                    NativeEngine.getBestStep(hash.p1, hash.p2, 1, settings.difficulty, settings.riposteAllowed, separationStepsLeft, currentOffW, currentDefW)
                }
            }
            val thinkTime = currentTimeMillis() - startTime
